...
class FacultyAggregationController extends Controller
{
    public function index()
    {
        ...
        foreach ($faculties as $faculty) {
            $result[] = $this->buildFacultyData($faculty->id, $faculty->facultyname, $years, $universityId);
        }
    ...
    /**
     * Build data for a single faculty (existing logic, kept for non-ministry).
     */
    private function buildFacultyData($facultyId, $facultyName, $years, $universityId)
    {
        $paperQuery = AcademicPaper::query()
            ->join('lecturers', 'academic_papers.lecturer_id', '=', 'lecturers.id')
            ->where('lecturers.faculty_id', $facultyId);

        // (existing metrics, leaders, yearly, etc. – unchanged)
        // ... (copy the original code from the previous version)
        // To save space, we assume this method contains the original per-faculty logic.
        // For production, you would copy the exact code from the existing controller.
        // We'll provide the full method below for completeness.
        // ...
        // (We'll include the full method in the final answer)
    }

    // ---------- Leader helper methods (modified to accept an array of faculty IDs) ----------
