<template>
  ...
  <!-- <div class="d-grid gap-3 grid-cols-2 mb-4">
    <radio-input btn-name="theme_scheme_direction" :img-component="true" label-title="LTR" class-name="text-center" label-class="p-0" id="theme-scheme-direction-ltr" :default-checked="themeSchemeDirection" value="ltr" @onChange="updateDirection">
    ...
    <radio-input btn-name="theme_scheme_direction" :img-component="true" label-title="RTL" class-name="text-center" label-class="p-0" id="theme-scheme-direction-rtl" :default-checked="themeSchemeDirection" value="rtl" @onChange="updateDirection">
...
export default {
  ...
  setup() {
    ...
    const themeScheme = computed(() => store.themeScheme)
    const themeSchemeDirection = computed(() => store.themeSchemeDirection)

    // Fix: only pass the value (the selected scheme)
    const updateRadio = (value) => {
      store.updateThemeScheme(value)
    }

    // For direction (already correct)
    const updateDirection = (value) => {
      store.updateThemeSchemeDirection(value)
    }
