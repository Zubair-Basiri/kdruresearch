<template>
  <div>
    ...
    <!-- Add click handler to reset to default navbar -->
    <div 
    ...
    >
      <span class="">Reset to Default Navbar</span>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const headerNavbar = computed(() => store.headerNavbar)
    const updateRadio = (value) => {
      store.updateHeaderNavbar(value)
    }
    // New method to reset navbar to default style
    const resetToDefault = () => {
      store.updateHeaderNavbar('navs-default')
    }
    return {
      headerNavbar,
      updateRadio,
